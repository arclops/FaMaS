...
function publicProduct(product) {
  const farmer = store.farmers.find((entry) => entry.fid === product.fid);
  const farmer_name = farmer
    ? `${farmer.fname || ''} ${farmer.lname || ''}`.trim()
    : product.farmer_name || '';
  return {
    ...product,
    image_url: product.image_url || product.img_url,
    farmer_name,
  };
}
...
const routes = [
  ...
  },
  {
    ...
    handle: async (match, options) => {
      ...
      const emailTaken = email
        ? store.accounts.some((account) => account.email && account.email.toLowerCase() === String(email).toLowerCase())
        : false;
      const phoneTaken = phone ? store.accounts.some((account) => account.phone === String(phone)) : false;
  ...
  },
  {
    ...
    handle: async (match) => {
      ...
      if (!account) return json({ error: 'No account for that e-mail/phone' }, 404);
  ...
  },
  {
    ...
    handle: async (match, options) => {
      ...
      if (!account) return json({ error: 'Unknown uid' }, 404);
  ...
  },
  {
    ...
    handle: async (match) => {
      ...
      if (!account && !farmer) return json({ data: {} });
      ...
      return json({
        data: {
          ...
          fname: source.fname || '',
          lname: source.lname || '',
          displayName: source.displayName || `${source.fname || ''} ${source.lname || ''}`.trim(),
          email: source.email || '',
          phone: source.phone || '',
  ...
  },
  {
    ...
    handle: async (match) => {
      ...
      if (!setFarmerStatus(fid, 'banned')) return json({ error: 'Unknown farmer' }, 404);
  ...
  },
  {
    ...
    handle: async (match) => {
      const fid = decodeURIComponent(match[1]);
      if (!setFarmerStatus(fid, 'active')) return json({ error: 'Unknown farmer' }, 404);
  ...
  },
  {
    ...
    handle: async (match) => {
      ...
      const products = isAdmin ? store.products : store.products.filter((product) => product.fid === uid);
  ...
  },
  {
    ...
    handle: async () => json({ data: store.products.filter((product) => product.stock > 0).map(publicProduct) }),
  },
  {
    method: 'POST',
