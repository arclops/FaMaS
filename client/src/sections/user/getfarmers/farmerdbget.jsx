...
export default async function getFarmerDb() {
    async function fetchsize(id) {
      try {
        const response = await apiFetch(`/api/admin/farmers/farmsize/${id}`);
    ...
    async function fetchfarms(id) {
      try {
        const response = await apiFetch(`/api/admin/farmers/farms/${id}`);
        const jsonData = await response.json();
    ...
    try {
      const response = await apiFetch(`/api/admin/farmers`, {
        method: "GET",
        headers: {
