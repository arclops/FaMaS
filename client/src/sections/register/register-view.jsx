...
export default function RegistrationView() {
  ...
  const [openRS,handleCloseRS]=useState(false);
  const [openRE, handleCloseRE] = useState(false);
  const [openREE, handleCloseREE] = useState(false);
  ...
  const handleRegister = async (e) => {
    ...
    let phone;
    const role = null;
    if(showEmail) {email = document.getElementsByName("email")[0].value};
    if(!showEmail) {phone = document.getElementsByName("phone")[0].value};
    // Validate registration data and send it to the server
    if (fname && lname && (email  || phone) && password === confirmPassword) {
      if(showEmail) phone = null;
      ...
      try{
        ...
        if (response.status === 201) {
          ...
          console.error('User with that email/phone exists!');
          handleCloseREE(true);
        } else {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Modals Section */}
        < RegSuccess openRS={openRS} handleCloseRS={() => {handleCloseRS(false);router.push('/dashboard')}} />
        < RegError openRE={openRE} handleCloseRE={() => {handleCloseRE(false);}} />
        < Regexistserror openREE={openREE} handleCloseREE={() => {handleCloseREE(false);}} />
      </Card>
    </Box>
  );
}
